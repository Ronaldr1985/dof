...
get_old_files :: proc(directory: string, t: time.Time) -> ([dynamic]string, os.Errno) {
	...
	for fi in fil {
		...
		relative_path = fmt.sprintf("%s/%s", directory, fi.name)
...
main :: proc() {
	...
	if len(os.args) == 3 {
		...
		*/
		...
		for ch in time_argument { // The first argument is the binary, the second is always going to be the time
